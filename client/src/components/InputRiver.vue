<template>
  <div class="buy-in"
       v-show="showInputRiver">
    <!-- <div class="shadow"
         @click="closeInputRiver"></div> -->
    <div class="buy-in-body">
      ...
      <div class="btn"><span @click="inputRiver">确定</span></div>
    </div>
  </div>
</template>
...
<script lang="ts">
  ...
  export default class InputRiver extends Vue {
    // prop
    @Prop() private showInputRiver!: boolean;
    ...
    private closeInputRiver() {
      this.$emit('update:showInputRiver', false);
    }

    private async inputRiver() {
      ...
      this.$emit('TransferRiverToRecord', this.River);
      // this.$emit('TransferIsHeroToCustomSitList', this.IsHero);
      this.closeInputRiver();
    }
    private mounted() {
